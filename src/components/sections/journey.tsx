                ...
                {journeyData.map((item, index) => (
                    <div key={index} className="relative mb-12">
                        <div className="md:flex items-center md:justify-between">
                            ...
                            {/* Content */}
                            <div className={`glass-card rounded-2xl p-6 w-full ml-12 md:ml-0 md:w-[45%] ${index % 2 === 0 ? 'md:order-1 md:text-left' : 'md:text-right'}`}>
                               <p className="text-lg font-semibold text-blue-400 mb-1">{item.period}</p>
                               <h3 className="text-2xl font-bold gradient-text">{item.college}</h3>
                               <p className="text-md text-gray-300 font-semibold mt-1">{item.degree}</p>
                           </div>
                        </div>
                    </div>
                ))}
